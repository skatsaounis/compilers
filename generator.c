...
void generate(Interpreted_quad quad, FILE * fp, int offset){
    ...
    if (strcmp(quad.quad, ":=") == 0){
        ...
        print_call_table(fp, current_unit, call_counter, offset, param_byte_table, inception_function_table, prev_param_offset_table);
		counterg++;
    }
...
void print_call_table(FILE * fp, char * fun_name, int call_counter, int temp_var_offset, int * param_byte_table, int * inception_function_table, char ** prev_param_offset_table){
    /* this needs fixing */
    int temp_number;
    /* this needs fixing */
